...
`doIMRMC` function which provides these results:

1.  `perReader`: per-reader AUC results

1.  `Ustat`: reader-averaged AUC results using U-statistics

1.  `ROC`: modality-specific ROC curves, and

1.  `varDecomp`: 5 variance decompositions for the reader-averaged AUCs.
