...
The following is an example demonstrating the use of the function `laWRBM.anova` to compute the WRBM limits of agreement for simulated MRMC agreement data. The example starts by simulating MRMC agreement data with the function `sim.NormalIG.Hierarchical` and finishes by printing the output.
...
The output is a list of two objects. The first object in the list is a data frame with key summary statistics. In order, they are the mean difference, the variance of the mean difference, the variance of WRBM differences $\hat V_{WR}^{12}$ (1 observation), the confidence interval of the mean difference, and the limits of agreement. 
...
The mixed effect model we applied is the unrestricted mixed model. Compared to the restricted model, the interaction term of the mixed effect and random effect in the unrestricted model will not be correlated at the same random level. This makes the model easier to work with.
...
The following is an example demonstrating the use of the function `laBRBM.anova` to compute the BRBM limits of agreement for simulated MRMC agreement data. The example uses the previously simulated MRMC agreement data. The output is a list of the same two objects as for `laWRBM`.
...
\]
Now, if we insert in the unbiased estimate of the variance components, we get the estimate of $V_{WRBM}$
\[
...
\end{aligned}
\]
The last term on the right-hand-side of the fomula above can be decomposed as
\[
\begin{aligned}
