...
%      figure(i*2-1)
     ...
     hold on
     for l = 1:length(studygrouplist)
        plot(1:size(sortNormal,2),sortNormal(l,:),strjoin([colorlist(3),signlist(l)],''),'MarkerSize',8);
     end
     LE = legend('fully crossed','2 split groups','3 split gorups');
...
subplot(3,1,1);
title({'MC mean Normal Assumption Observed Type 1 Error Rates'},'FontSize',16);
     dim = [.4 .8 .3 .1];
