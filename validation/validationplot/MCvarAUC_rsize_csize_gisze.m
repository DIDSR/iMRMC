    ...
    for j = 1: length(casesizelist)
       ...
       if ~isempty(rsize_csize_group)
           for k = 1: length(studygrouplist)
               rsize_csize_sg_group = rsize_csize_group(find(rsize_csize_group(:,studygroupcol)==studygrouplist(k)),:);
               if ~isempty(rsize_csize_sg_group)
                   ...
                   hold on
                   plotlegend = [plotlegend,['csize',num2str(casesizelist(j)),'studygroup',num2str(studygrouplist(k))] ];
               end
               a=a+1;
           end
       end
...
xlabel('reader size')
ylabel('mcVarAUC_A')
title('shape for different study group, color for different case size');
legend(plotlegend);
figure(2)
xlabel('reader size')
ylabel('mcVarAUC_B')
